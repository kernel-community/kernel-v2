...
You've finally reached the tranquil waters of Eth2 - congratulations! Like the network it 
describes, this post will be completed in some indefinite series of stages over the next 
"little while". Please check back here periodically for at least a few years.
...
In a way, this is what Kernel has been preparing you for: what Vinay Gupta 
called ["unimaginable strangeness"](/learn/module-1/promise-blockchains/#the-2010s-satoshis-vision), 
wherein we deploy a global, public, decentralized and censorship-resistant computing surface 
which anyone can use and no-one owns. 

What this [*means*](/learn/module-1/meaning/) for our networked species remains an 
open [question](/learn/module-2/better-questions/). One thing seems clear: it is an upgrade 
the likes of which is only seen once in an [age](/learn/module-2/debt/#virtual-money-coins-and-an-ancient-global-system). You'll need to recall the 
arguments made in [value](/learn/module-1/value/) and [incentives](/learn/module-5/incentives) 
to understand the design rationale for these next few scenes of 
the [human](/learn/module-0/conversation/) [story](/learn/module-5/listening-stories/).

In particular, [we claimed that](/learn/module-2/money-speech/#regulation-vs-expense) - 
with blockchains - the best way to protect free speech is to price it correctly. We don't 
enforce "the good" by legal fiat and deal with exceptions like libel and hate speech through 
human interpretation and violent enforcement; we define what is "bad" and set a price on it 
such that malicious expression is provably more expensive than its potential effects. Eth2 
extends this idea greatly: it is our 
...
for where the complexity goes is: layer 2 protocols → client implementations → protocol spec.
...
build, but an average person’s ECC keys are secure enough to resist even state-level actors.
...
around, for instance, block sizes which lead to sub-optimal outcomes for users of the network.
...
which define and encode what it means to cheat do no need to be [trusted](/learn/module-0/trust/), 
...
Q: The cost of attacking or defending Proof of Work consensus is 1:1. Proof of Stake breaks this symmetry by relying on what, instead of rewards? This solves the conflict of interest between miners and users, because Eth2 validators and Eth2 users are more likely to be what?
...
Griefing occurs when an attacker seeks to reduce other validators’ revenue, even at some 
...
incentivizes griefing more than can be disincentivzed by the collective scheme above. Again, 
...
Q: Making the base reward proportional to the inverse square root of all deposits strikes a balance between what two fixed quantities? If the reward rate were fixed, it would create too much uncertainty about what two things? If the total rewards were fixed, it would incentivize what kind of malicious behaviour?
...
Rewards are designed this way *only as a result* of the thinking about how to penalize 
...
Q: If everyone else is online, what percentage of time must I be online for to break even as a validator? If others start to go offline, does this increase or decrease?
...
What principle of the Design Rationale dictated the choice of a beacon chain, rather than distinct shards connected in some other manner?
...
What is Eth2? Well, we said it already: our generation's elder game of economic penalties. 
These penalties are the game mechanics we use to reveal 
a [unique kind of truth](/learn/module-5/reveal-the-universe/#the-value-of-truth): it is 
possible to build - and asymmetrically defend/maintain - an explicitly prosocial, global, and 
ownerless system that provably benefits all the people who choose to use it. The benefits of 
encoding penalties extend to all layers of this game, including our ability 
to <Link to="https://twitter.com/VitalikButerin/status/1294461236680130560" >use co-ordination costs to our advantage</Link>.

Welcome. Most of us [friends](/learn/module-5/prosocial-value/#friendship) here, 
