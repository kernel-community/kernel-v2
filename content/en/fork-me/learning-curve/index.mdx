...
<Image alt="Learning Curve Explainer" src="/images/Learning-Curve.png" />
...
> **string _url** - a simple "metadata" field to link any course to its frontend, useful for validation &amp; security, as well as future frontend displays;   
...
This method allows a learner to register for a course of their choice. It checks the fee associated with that course, accepts tokens amounting to that fee and registers the learner. The tokens are kept in the contract and assigned to the current batch. Once there are enough tokens in the batch to justify the gas costs of calling `batchDeposit()`, anyone may do so and add the current batch the yearn vault.
...
All course are deployed with a given number of checkpoints allowing learners to receive a portion of their fees back at various stages in the course. This is a helper function that checks where a learner is in a course and is used by both `redeem()` and `mint()` to figure out the proper amount required. It is public, so anyone may check it at any time.
...
If a learner is redeeming rather than minting, it means they are simply requesting their initial fee back (whether they have completed the course or not). In this case, it checks what proportion of `fee` (set when the course is deployed) must be returned and sends it back to the learner. Whatever yield they earned is sent to the course creator.

If the learner has not completed the course, it checks the period elapsed since `blockRegistered`, at which point the full `fee` can be returned and the yield sent to the course creator.
...
Handles learner minting new LEARN checks via `verify()` what proportion of the fee to send to the Learning Curve, adds any yield earned to that, and returns all the resulting LEARN tokens to the learner. This acts as an effective discount for learners (as they receive more LEARN) without us having to maintain a whitelist or the like: great for simplicity, security and usability
...
> params: **uint256 _daiToReceive** - how much underlying DAI the burner wishes to receive back.

Ideally, we'd simply pass in how much LEARN is to be burned. However, taking in a `_amountLearn` and then converting this on-chain to the `_daiToReceive` is not possible, as it requires dividing by `e` raised to the power of (_amountLearn / k). 
...
**getBurnableForReserveAmount**

> params: **uint256 reserveAmount** - DAI to receive

A helper function for the frontend so that we can calculate and pass the correct `_daiToReceive` to the `burn` method. It returns to the amount of LEARN which must be burnt in order to receive the requested DAI, which can be used to check if the learner has enough LEARN tokens for a successful `burn` transaction.
