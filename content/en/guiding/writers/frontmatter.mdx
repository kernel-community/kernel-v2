...
The "order" is **relative to the current directory**, so you can't take a page deeper in the directory and put it on the top level of the sidebar. For example, `/learn/module-1/value.mdx` with an order of 0 will just appear at the top of the `module-1` dropdown in the sidenav and not at the top of the `learn` sidenav.
