...
const BlogHome = ({data}) => {
    ...
    const latestPosts =
        initialSection !== null && initialSectionExists
            ? data.allMdx.edges
                  .filter(
                      ({node}) =>
                          getBlogPostTypeFromPath(node.fileAbsolutePath) ===
                          initialSection
                  )
                  .slice(0, 3)
            : types.length > 0
    ...
    const showNextButton =
        (sectionData.currentPage + 1) * postsPerPage <
        sectionData.allPosts.length;
    ...
    const setBlogCategory = (cat = null) => {
        ...
        if (cat === null) {
            ...
            allPosts = data.allMdx.edges.filter(
                ({node}) =>
                    getBlogPostTypeFromPath(node.fileAbsolutePath) === cat
            ...
            <Flex
                ...
                {types.map((type, index) => (
                    <Link
                        sx={{
                            color:
                                sectionData.type === type ? "link" : "mutedAlt",
                            cursor: "pointer",
                            transition: "all .16s",
                            fontWeight: 400,
