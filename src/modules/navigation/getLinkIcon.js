...
const LinkWrapper = ({ ariaLabel, ...props }) => {
  ...
  return (
    <div sx={{ backfaceVisibility: 'hidden', display: 'inline-block' }}>
      <Link
        hideExternalIcon
...
const getLinkIcon = (url, key) => {
  ...
  } else if (url.includes('kernel')) {
    return (
      <LinkWrapper to={url} key={key} ariaLabel={'kernel_icon'}>
        <Icon name="kernel_icon" size={'23px'} sx={{ color: 'textMuted' }} />
      </LinkWrapper>
    )
  }
